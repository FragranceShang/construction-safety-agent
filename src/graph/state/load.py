...
def load_memory_node(state: RegulationState) -> RegulationState:
    ...
    if history_items:
        history_str = "【历史对话】\n" + "\n\n".join(
            f"用户：{item['user']}\n助手：{item['assistant']}" for item in history_items
        )
    else:
        history_str = "【历史对话】\n（无历史对话）"
    ...
    if knowledge_items:
        ...
        for item in knowledge_items:
            knowledge_lines.append(
                "• 场景关键词：" + "、".join(item.get("extracted_keywords", []))
            )
            knowledge_lines.append(
                "• 相关条例：" + "、".join(item.get("matched_articles", []))
            )
    ...
    state["history"] = "\n\n".join([profile_str, history_str, knowledge_str])
