...
def inspect_parse_vision_node(state: InspectionState) -> InspectionState:
    ...
    inspection_logger.info(
        "inspect_parse_vision_node | image_path=%s, scene_parse_path=%s, has_scene_parse=%s",
        state.get("image_path"),
        state.get("scene_parse_path"),
        state.get("scene_parse"),
    )

    if state.get("scene_parse"):
        scene = SceneParseResult.model_validate(state["scene_parse"])
        state["scene_parse"] = scene.model_dump()
        state["vision_text"] = json.dumps(
            state["scene_parse"], ensure_ascii=False, indent=2
        )
        return state
